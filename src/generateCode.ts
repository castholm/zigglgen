...
export function generateCode(features: ResolvedFeatures, apiVersionProfile: string): string {
  ...
  sb.push("\n")
  sb.push("/// Static information about the OpenGL binding and how its source code was generated.\n")
  ...
  sb.push("};\n")
  sb.push("\n")
  sb.push("/// Initializes the OpenGL binding. This function must be called before calling any other functions\n")
  sb.push("/// declared by the binding.\n")
  sb.push("///\n")
  ...
  sb.push("/// const AnyCommandFnPtr = *align(@alignOf(fn () callconv(.C) void)) const anyopaque;\n")
  sb.push("/// const fn_ptr: ?AnyCommandFnPtr = loader.GetCommandFnPtr(command_name);\n")
  sb.push("/// _ = fn_ptr;\n")
  if (hasExtensions) {
    sb.push("///\n")
  ...
  sb.push("/// ```\n")
  sb.push("///\n")
  sb.push("/// No references to `loader` are retained by the binding after this function returns.\n")
  sb.push("pub fn init(loader: anytype) void {\n")
  sb.push("    features.init(loader);\n")
  sb.push("}\n")
  sb.push("\n")
  if (hasExtensions) {
    ...
    sb.push('    return @field(features, "GL_" ++ @tagName(extension));\n')
    sb.push("}\n")
    sb.push("\n")
    sb.push("/// Extensions known to the OpenGL binding.\n")
  ...
  for (const command of features.commands.values()) {
    ...
    sb.push(`    return features.${zigIdentifier(command.key)}`)
    if (command.optional) {
      sb.push(".?")
    }
    sb.push(`(`)
  ...
  sb.push("\n")
  sb.push("/// Currently loaded OpenGL features.\n")
  sb.push("pub var features: Features = undefined;\n")
  sb.push("\n")
  sb.push("/// Holds dynamically loaded OpenGL features.\n")
  sb.push("pub const Features = struct {\n")
  for (const extension of features.extensions.values()) {
  ...
  sb.push("\n")
  sb.push("    pub fn init(f: *Features, loader: anytype) void {\n")
  ...
  for (const extension of features.extensions.values()) {
    ...
    for (const command of extension.commands.map(x => features.commands.get(x)!)) {
      sb.push(`            f.loadCommand(loader, "${zigIdentifier(command.key)}");\n`)
    }
    sb.push(`            f.${zigIdentifier(extension.key)} = true;\n`)
    sb.push(`        }\n`)
  }
  sb.push("    }\n")
  sb.push("\n")
  sb.push("    fn nullTerminate(comptime str: []const u8) [:0]const u8 {\n")
  sb.push("        comptime {\n")
  sb.push("            var buf: [str.len + 1]u8 = undefined;\n")
  sb.push("            std.mem.copy(u8, &buf, str);\n")
  sb.push("            buf[str.len] = 0;\n")
  sb.push("            return buf[0..str.len :0];\n")
  sb.push("        }\n")
  sb.push("    }\n")
  sb.push("\n")
  sb.push("    fn loadCommand(f: *Features, loader: anytype, comptime name: [:0]const u8) void {\n")
  sb.push("        const AnyCommandFnPtr = *align(@alignOf(fn () callconv(.C) void)) const anyopaque;\n")
  sb.push("        const fn_ptr: ?AnyCommandFnPtr = loader.getCommandFnPtr(name);\n")
  sb.push("        @field(f, name) = @ptrCast(@TypeOf(@field(f, name)), fn_ptr);\n")
  sb.push("    }\n")
  sb.push("};\n")
  sb.push("\n")
  sb.push("test {\n")
